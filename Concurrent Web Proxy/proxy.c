...
pthread_mutex_t lockConnection = PTHREAD_MUTEX_INITIALIZER;
...
void format_log_entryP(char *logstring, struct sockaddr_in *sockaddr, char *uri, int size);

/* 
 * main - Main routine for the proxy program 
 */
 struct sockaddr_in clientaddr;  /* Clinet address structure*/  //need it for format_log_entry
...
void *RunConnection(void *fd){
    ...
    //struct sockaddr_in clientaddr;  /* Clinet address structure*/  //needs to be global due to format_log_entry and its from main to thread handler method
    ...
    int realloc_factor;         /* Total size of HTTP request */
    ...
    //lock here for thread synchronization
    pthread_mutex_lock(&lockConnection);
    format_log_entry(log_entry, &clientaddr, request_uri, response_len);  //time in the method is not thread safe
    fprintf(log_file, "%s %d\n", log_entry, response_len);
    fflush(log_file);
    pthread_mutex_unlock(&lockConnection);
    //unlock here so that next thread can pass through
    close(connfd);
    close(serverfd);
    free(fd);
    free(request); //might have to comment out to avoid double free error
   
	return NULL;
} 
 
int ThreadMain(int portNum){
	 int listenfd;             /* The proxy's listening descriptor */
    int port;                 /* The port the proxy is listening on */
    int clientlen;            /* Size in bytes of the client socket address */
    int request_count = 0;    /* Number of requests received so far */
      /* Clinet address structure*/  
    int connfd;                     /* socket desciptor for talkign wiht client*/ 
    int serverfd;                   /* Socket descriptor for talking with end server */
    char *request;                  /* HTTP request from client */
    char *request_uri;              /* Start of URI in first HTTP request header line */
    ...
    */
    //else{ 
    pthread_create(&tid, NULL, RunConnection,&connfd);
    pthread_detach(tid);
	 //}
...
int ProcessMain(int portValue)
{
     ...
     */
    // Make the flock structure.
    struct flock lock;
	 memset (&lock, 0, sizeof(lock));
	 lock.l_type = F_WRLCK;
	 /* Place a write lock on the whole file. */	
	 //printf ("blocking lock on file\n");
    ...
    close(connfd); //close it in parent and not the child
    close(serverfd);
	}
		      ...
		      char *uri, int size)
{
    ...
    //pthread_mutex_lock(&lockConnection);//want to write to it one at a time but already locked in thread handler
    sprintf(logstring, "%s: %d.%d.%d.%d %s", time_str, a, b, c, d, uri);
    //pthread_mutex_lock(&lockConnection);
}

void format_log_entryP(char *logstring, struct sockaddr_in *sockaddr, 
		      char *uri, int size) //used for multi processes instead of threads
{
    time_t now;
    char time_str[MAXLINE];
    unsigned long host;
